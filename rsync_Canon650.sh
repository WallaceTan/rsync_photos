...
echo /usr/bin/rsync -rt --progress --include-from="${RSYNC_INCLUDE}" "$CWD/" "$TARGET_DIR/"
/usr/bin/rsync -rt --progress --include-from="${RSYNC_INCLUDE}" "$CWD/" "$TARGET_DIR/"
...
# Skip deleting files from SD-CARD
unset IFS
exit
